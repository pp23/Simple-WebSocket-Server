...
int main() {
    ...
    //    ws.onmessage=function(evt){console.log(evt.data);};
    //    ws.send("test");
    auto& echo=server.addRuntimeEndpoint("^/echo/?$");
    ...
    echo.onmessage=[&server](shared_ptr<WsServer::Connection> connection, shared_ptr<WsServer::Message> message) {
        ...
        cout << "Server: /echo: Message received: \"" << message_str << "\" from " << (size_t)connection.get() << endl;

        cout << "Server: /echo: Sending message \"" << message_str <<  "\" to " << (size_t)connection.get() << endl;
